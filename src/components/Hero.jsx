  ...
  return (
    <section id="hero" className="relative isolate overflow-hidden px-4 py-28 sm:px-6 lg:px-8 lg:py-36">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-400">
            I build practical ML, GenAI, and automation systems that turn complex workflows into useful products.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            <a href="https://drive.google.com/file/d/14KZOgVekKO5X_Z-MFw-m-qz2cQ9-8Y0Q/view?usp=sharing" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-slate-400/60 bg-white/70 px-5 py-3 font-semibold text-slate-800 transition hover:scale-105 dark:bg-slate-900/70 dark:text-slate-100">
              Download CV <FiDownload />
            </a>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="relative">
